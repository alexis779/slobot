...
class SimPolicy:
    ...
    def execute(self, recording_layout: RecordingLayout) -> bool:
        ...
        #self.golf_ball_env.arm.genesis.side_camera.stop_recording("./so_arm_100.mp4", fps=self.golf_ball_env.arm.genesis.fps)
    ...
    def ik_path_plan(self, target_pos, target_quat, gripper_qpos):
        ...
        if angular_error_rad > rot_eps:
            raise ValueError(f"Orientation error is too large (tolerance: {rot_eps:.4f} rad = {torch.rad2deg(torch.tensor(rot_eps)):.2f}°)")
        ...
        if pos_error > pos_eps:
            raise ValueError(f"Position error is too large (tolerance: {pos_eps:.4f} m)")

    def plan_path(self, qpos, gripper_qpos):
        path = self.golf_ball_env.arm.genesis.entity.plan_path(
            qpos,
        )
