...
class PytorchSolver:
    ...
    def transform_by_quat(self, vecs, quats):
        ...
        # Apply rotation: (N, 3, 3) @ (N, 3) -> (N, 3)
        result = torch.bmm(R, vecs.unsqueeze(-1)).squeeze(-1)
    ...
    def compute_link_quat_pos(self, pos):
        ...
        link_quat = torch.zeros((dofs, Configuration.NUM_DIMS_QUAT), device=self.device)
        link_pos = torch.zeros((dofs, Configuration.NUM_DIMS_3D), device=self.device)

        link_quat0 = self.link_initial_quat.clone()
        link_rel_pos = self.link_initial_pos.clone()
